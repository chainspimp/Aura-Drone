...
Usage:
    python drone_main.py [--no-gui] [--sim]
...
class AURADrone:
    ...
    def _generate_situation_report(self) -> None:
        ...
        if self.tts:
            self.tts.speak(report[:500])  # Truncate to reasonable spoken length
    ...
    def shutdown(self) -> None:
        ...
        # Stop subsystems
        for subsystem in [self.yolo, self.thermal, self.speech_input, self.wake_listener, self.lora]:
